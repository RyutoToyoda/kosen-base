...
import { 
  BookOpen, Search, Plus, Settings, Send, LayoutDashboard, FileText, 
  Calendar as CalendarIcon, MoreVertical, BrainCircuit, GraduationCap,
  Loader2, ImagePlus, AlertCircle, CheckCircle2, Clock, ChevronRight, 
  Trash2, LogOut, User, ArrowLeft, X, Compass, Bookmark, Filter
} from 'lucide-react';
...
const INITIAL_CHAT = [{ id: 1, sender: 'ai', text: 'KOSEN-base AIです。学習の相談や過去問分析など、何でも聞いてください。' }];

const getItemType = (tags) => Array.isArray(tags) ? tags.find(t => t.startsWith('type:'))?.split(':')[1] || 'note' : 'note';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    initAuth();
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => handleSessionUpdate(session));
  ...
  const checkReady = () => {
    ...
    if (!hasEnvVars) throw new Error("環境変数が読み込めていません。VercelでRedeployをしてください。");
  };

  const showMsg = (type, text) => {
    setAnalyzeMessage({ type, text });
    setTimeout(() => setAnalyzeMessage({ type: null, text: null }), type === 'success' ? 4000 : 7000);
  };
    ...
    try {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`, {
        ...
        body: JSON.stringify({ contents: [{ role: "user", parts: [
          { text: "高専の学習ノート画像を解析し、JSON形式で返してください。\n{\n  \"title\": \"\",\n  \"subject\": \"\",\n  \"preview\": \"詳細な要約\",\n  \"tags\": []\n}" },
      ...
      if (error) throw error;
      await fetchNotes();
      showMsg('success', '画像からノートを作成しました！');
    } catch (err) { showMsg('error', err.message); } 
  ...
  const handleAnalyzeAI = async (item) => {
    ...
    try {
      ...
      const prompt = type === 'exam' 
        ? `高専の教員として、この過去問（${item.title}）から出題分野を特定し、解法のポイントを3点教えてください。`
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: `高専生専用AIとして回答してください。質問: ${userText}` }] }] })
      });
      const result = await response.json();
  ...
  const getCalendar = () => {
    ...
    for (let i = 1; i <= last.getDate(); i++) days.push({ day: i, dateStr: `${year}-${String(month+1).padStart(2,'0')}-${String(i).padStart(2,'0')}` });
  ...
  const menuItems = [
    ...
    { id: 'materials', label: 'Materials', icon: Bookmark },
  ...
  if (!session) {
    return (
      <div className="flex h-screen w-full bg-[#0a0f18] text-slate-200 items-center justify-center relative overflow-hidden">
        <div className="absolute w-[600px] h-[600px] bg-emerald-500/5 blur-[120px] rounded-full animate-pulse"></div>
        <div className="w-full max-w-md bg-[#0d1424]/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-10 shadow-2xl relative z-10 mx-4">
          ...
          <form className="space-y-5" onSubmit={(e) => {
            ...
            <div className="flex bg-[#161f33] p-1.5 rounded-xl mb-6 shadow-inner">
              <button type="button" onClick={() => setIsLoginMode(true)} className={`flex-1 py-2.5 text-xs font-black uppercase rounded-lg transition-all ${isLoginMode ? 'bg-slate-700 text-white shadow-md' : 'text-slate-500'}`}>LOGIN</button>
              <button type="button" onClick={() => setIsLoginMode(false)} className={`flex-1 py-2.5 text-xs font-black uppercase rounded-lg transition-all ${!isLoginMode ? 'bg-slate-700 text-white shadow-md' : 'text-slate-500'}`}>SIGN UP</button>
            </div>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-[#161f33] border border-slate-700 text-slate-200 rounded-xl px-5 py-4 text-sm focus:border-emerald-500 font-bold" placeholder="Email Address" />
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-[#161f33] border border-slate-700 text-slate-200 rounded-xl px-5 py-4 text-sm focus:border-emerald-500 font-bold" placeholder="Password" />
            {authError && <div className="text-red-400 text-xs flex items-start bg-red-400/10 p-3 rounded-lg"><AlertCircle className="w-4 h-4 mr-2 shrink-0" />{authError}</div>}
            <button type="submit" disabled={isAuthSubmitLoading} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-4 rounded-xl font-black transition-all shadow-lg active:scale-95 flex justify-center uppercase text-sm tracking-widest">
              {isAuthSubmitLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : (isLoginMode ? 'Sign In' : 'Create Account')}
            </button>
          </form>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 sm:p-8 animate-in fade-in" onClick={() => setSelectedNote(null)}>
          <div className="bg-[#0d1424] border border-slate-700 rounded-3xl p-6 sm:p-10 w-full h-full max-w-5xl shadow-2xl relative flex flex-col overflow-hidden text-left" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto pr-2 scrollbar-hide space-y-8">
              <section>
                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center"><FileText className="w-4 h-4 mr-2" /> Description</h4>
              ...
              <section className="bg-[#161f33] border border-emerald-500/10 rounded-3xl p-6 sm:p-8 shadow-inner">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                  ...
                  {!relevanceAnalysis.text && !relevanceAnalysis.loading && (
                    <button onClick={() => handleAnalyzeAI(selectedNote)} className="text-xs font-black bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl flex items-center shadow-lg transition-all active:scale-95 uppercase tracking-widest"><Compass className="w-4 h-4 mr-2" /> Start</button>
                  )}
                </div>
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/80 backdrop-blur-xl p-4 animate-in fade-in text-left">
          <div className="bg-[#0d1424] border border-slate-700 rounded-3xl p-8 w-full max-w-md shadow-2xl relative animate-in zoom-in-95">
            <div className="w-16 h-16 bg-emerald-500/10 rounded-2xl flex items-center justify-center mb-6 mx-auto"><User className="w-8 h-8 text-emerald-500" /></div>
            <h2 className="text-2xl font-black text-white text-center mb-6">Profile Setup</h2>
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              ...
              <button type="submit" disabled={isProfileUpdating} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-4 rounded-xl font-black mt-6 flex justify-center uppercase tracking-widest text-xs">
                {isProfileUpdating ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Enter App'}
              </button>
            </form>
          </div>
        </div>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in" onClick={() => setIsAddModalOpen(false)}>
          <div className="bg-[#0d1424] border border-slate-700 rounded-3xl p-8 w-full max-w-lg shadow-2xl relative animate-in zoom-in-95 text-left" onClick={e => e.stopPropagation()}>
            <h2 className="text-xl font-black text-white mb-6 flex items-center uppercase tracking-widest"><Plus className="w-6 h-6 mr-3 text-emerald-500" /> New Item</h2>
            <div className="flex bg-[#161f33] p-1.5 rounded-xl mb-6 shadow-inner border border-slate-800">
              {['note', 'exam', 'material'].map(t => (
                <button key={t} onClick={() => setNewItemType(t)} className={`flex-1 py-2.5 text-[10px] font-black uppercase rounded-lg transition-all ${newItemType === t ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-500'}`}>
            ...
            <form onSubmit={handleManualAdd} className="space-y-4">
              ...
              <div className="flex gap-4 pt-4">
                <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 bg-[#161f33] text-slate-500 py-3.5 rounded-xl font-black border border-slate-700 text-xs tracking-widest uppercase">Cancel</button>
      ...
      {isEventModalOpen && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in" onClick={() => setIsEventModalOpen(false)}>
          <div className="bg-[#0d1424] border border-slate-700 rounded-3xl p-8 w-full max-w-sm shadow-2xl text-left" onClick={e => e.stopPropagation()}>
            <h2 className="text-xl font-black text-white mb-6 flex items-center uppercase tracking-tight"><CalendarIcon className="w-6 h-6 mr-3 text-emerald-500" /> New Event</h2>
            <p className="text-[10px] text-slate-500 mb-6 font-mono font-black tracking-widest uppercase">{selectedDate}</p>
            <form onSubmit={handleAddEvent} className="space-y-4">
              <input type="text" autoFocus required value={newEventTitle} onChange={e => setNewEventTitle(e.target.value)} className="w-full bg-[#161f33] border border-slate-700 text-slate-200 rounded-xl px-5 py-4 text-sm focus:border-emerald-500 font-bold" placeholder="予定内容" />
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setIsEventModalOpen(false)} className="flex-1 bg-[#161f33] text-slate-500 py-3 rounded-xl font-black border border-slate-700 text-[10px] uppercase tracking-widest">Back</button>
                <button type="submit" className="flex-1 bg-emerald-600 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest">Save</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <aside className="w-64 bg-[#0d1424] border-r border-slate-800 flex flex-col hidden md:flex z-20 shrink-0 text-left">
        ...
        <div className="p-6 border-t border-slate-800 shrink-0">
          <button onClick={() => { setActiveView('settings'); setSelectedSubject(null); }} className={`w-full flex items-center px-5 py-3 text-[10px] font-black uppercase tracking-widest transition-all group mb-4 rounded-lg ${activeView === 'settings' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-800/50'}`}>
            <Settings className="w-4 h-4 mr-4 group-hover:rotate-90 transition-transform" />SETTING
          </button>
          ...
          <p className="text-[9px] text-slate-800 mt-6 text-center font-mono font-black tracking-[0.3em]">VER 1.2.2 FIXED</p>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#0a0f18] overflow-hidden relative z-20 text-left">
        <header className="h-20 flex items-center justify-between px-10 border-b border-slate-800 bg-[#0d1424]/80 backdrop-blur-xl shrink-0">
          <div className="flex-1 max-w-xl relative group">
            ...
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search records..." className="w-full bg-[#161f33] border border-slate-700 text-slate-200 rounded-xl pl-14 pr-6 py-3.5 focus:outline-none focus:border-emerald-500 text-sm font-bold shadow-inner placeholder:text-slate-700" />
          </div>
          <div className="ml-8 flex items-center gap-4 shrink-0">
            ...
            <button onClick={() => fileInputRef.current?.click()} disabled={isAnalyzing} className="flex items-center bg-[#161f33] hover:bg-slate-700 text-slate-400 border border-slate-700 px-6 py-3.5 rounded-xl font-black transition-all disabled:opacity-50 text-[10px] uppercase tracking-widest shadow-lg active:scale-95">
              {isAnalyzing ? <Loader2 className="w-4 h-4 mr-3 animate-spin text-emerald-500" /> : <ImagePlus className="w-4 h-4 mr-3 text-emerald-500" />} Analyze
            </button>
            <button onClick={() => setIsAddModalOpen(true)} className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-3.5 rounded-xl font-black shadow-lg active:scale-95 text-[10px] uppercase tracking-widest flex items-center"><Plus className="w-4 h-4 mr-2" /> New Entry</button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-10 scrollbar-hide text-left">
          ...
          {activeView === 'dashboard' && (
            <div className="max-w-6xl mx-auto animate-in fade-in duration-500">
              <h2 className="text-2xl font-black text-white flex items-center mb-10 uppercase tracking-widest"><LayoutDashboard className="w-6 h-6 mr-4 text-emerald-500" /> Recent Activity</h2>
              {isLoading ? (
            ...
            <div className="max-w-6xl mx-auto animate-in slide-in-from-bottom duration-500">
              {!selectedSubject ? (
                <div className="text-center py-12">
                  ...
                  <h2 className="text-3xl font-black text-white mb-4 uppercase tracking-widest">Note Archives</h2>
                  <p className="text-slate-600 mb-12 font-black tracking-widest uppercase text-xs">Grouped by Subject</p>
                    ...
                    {Array.from(new Set(filteredItems.filter(n => getItemType(n.tags) === 'note').map(n => n.subject))).map(sub => (
                      <div key={sub} onClick={() => setSelectedSubject(sub)} className="p-8 bg-[#0d1424] border border-slate-800 rounded-3xl hover:border-emerald-500/40 transition-all flex items-center justify-between cursor-pointer group shadow-lg">
                        <div className="flex items-center">
                          ...
                          <div>
                            <p className="font-black text-slate-100 text-lg tracking-tight mb-1">{sub || 'Global'}</p>
                ...
                <div className="animate-in fade-in duration-500 text-left">
                  <button onClick={() => setSelectedSubject(null)} className="flex items-center text-slate-600 hover:text-emerald-400 font-black text-xs mb-10 transition-all uppercase tracking-widest group"><ArrowLeft className="w-5 h-5 mr-3 group-hover:-translate-x-2 transition-transform" /> Back to Root</button>
                  <h2 className="text-3xl font-black text-white mb-10 border-l-8 border-emerald-500 pl-6 uppercase">{selectedSubject}</h2>
          ...
          {activeView === 'exams' && (
            <div className="max-w-6xl mx-auto animate-in slide-in-from-bottom duration-500 text-left">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-12 gap-6">
                <h2 className="text-3xl font-black text-white flex items-center uppercase tracking-widest"><FileText className="w-8 h-8 mr-4 text-red-500" /> Exam Core</h2>
                <div className="flex bg-[#161f33] border border-slate-700 rounded-2xl p-1.5 shadow-lg min-w-[360px]">
                  ...
                  <select value={examFilter.grade} onChange={e => setExamFilter({...examFilter, grade: e.target.value})} className="bg-transparent text-slate-400 text-[10px] font-black uppercase px-4 py-2 outline-none cursor-pointer flex-1"><option value="">Grade</option><option value="1年">1st</option><option value="2年">2nd</option><option value="3年">3rd</option><option value="4年">4th</option><option value="5年">5th</option></select>
                  <select value={examFilter.term} onChange={e => setExamFilter({...examFilter, term: e.target.value})} className="bg-transparent text-slate-400 text-[10px] font-black uppercase px-4 py-2 outline-none cursor-pointer border-l border-slate-700 flex-1"><option value="">Term</option><option value="前期">Pre</option><option value="後期">Post</option></select>
                  <select value={examFilter.type} onChange={e => setExamFilter({...examFilter, type: e.target.value})} className="bg-transparent text-slate-400 text-[10px] font-black uppercase px-4 py-2 outline-none cursor-pointer border-l border-slate-700 flex-1"><option value="">Exam</option><option value="中間">Mid</option><option value="期末">Final</option><option value="小テスト">Mini</option></select>
                </div>
              </div>
          ...
          {activeView === 'materials' && (
            <div className="max-w-6xl mx-auto animate-in fade-in duration-500 text-left">
              <h2 className="text-3xl font-black text-white flex items-center mb-12 uppercase tracking-widest"><Bookmark className="w-8 h-8 mr-4 text-blue-500" /> Digital Library</h2>
          ...
          {activeView === 'calendar' && (
            <div className="max-w-6xl mx-auto animate-in slide-in-from-top duration-700 text-left">
              <h2 className="text-3xl font-black text-white flex items-center mb-10 uppercase tracking-widest"><CalendarIcon className="w-8 h-8 mr-4 text-emerald-500" /> Scheduler <span className="ml-6 text-sm text-slate-600 font-mono tracking-[0.3em]">{calendar.year}.{String(calendar.month + 1).padStart(2,'0')}</span></h2>
          ...
          {activeView === 'settings' && (
            <div className="max-w-2xl mx-auto py-12 animate-in fade-in duration-500 text-left">
              <h2 className="text-3xl font-black text-white mb-10 flex items-center uppercase tracking-widest"><Settings className="w-8 h-8 mr-4 text-emerald-500" /> Settings</h2>
              <div className="bg-[#0d1424] border border-slate-800 rounded-3xl p-10 shadow-xl border-t-emerald-500/30">
                <form onSubmit={handleUpdateProfile} className="space-y-8">
                  <div className="space-y-2"><label className="block text-[10px] font-black text-slate-600 uppercase tracking-widest ml-1">Account ID</label><input type="text" disabled value={session.user.email} className="w-full bg-[#161f33]/50 border border-slate-800 text-slate-500 rounded-xl px-6 py-4 text-sm font-mono font-black" /></div>
                  <div className="space-y-2"><label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">College Campus</label><input required type="text" value={profileForm.kosen} onChange={e => setProfileForm({...profileForm, kosen: e.target.value})} className="w-full bg-[#161f33] border border-slate-700 text-slate-200 rounded-xl px-6 py-4 text-sm focus:border-emerald-500 font-bold" /></div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2"><label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Major</label><input required type="text" value={profileForm.department} onChange={e => setProfileForm({...profileForm, department: e.target.value})} className="w-full bg-[#161f33] border border-slate-700 text-slate-200 rounded-xl px-6 py-4 text-sm focus:border-emerald-500 font-bold" /></div>
                    <div className="space-y-2"><label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Grade</label>
                      <select required value={profileForm.grade} onChange={e => setProfileForm({...profileForm, grade: e.target.value})} className="w-full bg-[#161f33] border border-slate-700 text-slate-200 rounded-xl px-6 py-4 text-sm outline-none font-bold">
                        <option value="1年">1st Year</option><option value="2年">2nd Year</option><option value="3年">3rd Year</option><option value="4年">4th Year</option><option value="5年">5th Year</option><option value="専攻科">Adv.</option>
                      </select>
                    </div>
                  </div>
                  <div className="pt-8 flex justify-end border-t border-slate-800">
                    <button type="submit" disabled={isProfileUpdating} className="bg-emerald-600 hover:bg-emerald-500 text-white px-10 py-4 rounded-xl font-black shadow-lg active:scale-95 flex items-center justify-center min-w-[200px] uppercase text-[10px] tracking-widest">Save Identity</button>
                  </div>
                </form>
              </div>
            </div>
