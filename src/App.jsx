...
// 一番安定して動作していた読み込み方（+ Vite静的置換もサポート）
const getEnvVar = (key) => {
  try {
...
export default function App() {
  ...
  const menuItems = [
    ...
    { id: 'exams', label: '過去問・資料', icon: FileText },
            ...
            <div className="max-w-7xl mx-auto animate-in fade-in duration-500">
              ...
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredNotes.slice(0, 12).map(renderCard)}
                </div>
              )}
            </div>
          )}
            ...
            <div className="max-w-7xl mx-auto animate-in slide-in-from-bottom duration-500">
              {!selectedSubject ? (
                ...
                <div className="animate-in fade-in duration-300">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredNotes.filter(n => getItemType(n.tags) === 'note' && n.subject === selectedSubject).map(renderCard)}
                  </div>
                </div>
              )}
            </div>
